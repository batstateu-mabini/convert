        ...
        $.each(window.arrayOfPdf, function(i, v) {
            $('.list-files').append(`
                <div id="pdf-${i}" class="pdf-file">
                    ...
                    <img style="height: 75px" src="path_to_pdf_icon_placeholder">
                    <br>${v.name}
                </div>`);
        });
    } else {
...
// Convert images to a PDF and return the PDF as bytes
function convertImagesToPdf() {
    return new Promise((resolve, reject) => {
        ...
        imageFiles.forEach((file, index) => {
            ...
            reader.onload = () => {
                ...
                img.onload = () => {
                    ...
                    pdf.addImage(img, 'JPEG', 10, 10, pageWidth, imageHeight);
        ...
        if (imageFiles.length === 0) {
            resolve(null); // No images to process
        }
    });
}

// Combine PDFs and Images to a single PDF
async function combineToPdf() {
    ...
    // Convert images to a PDF and merge it
    const imagePdfBytes = await convertImagesToPdf();
    if (imagePdfBytes) {
        const imgPdf = await PDFDocument.load(imagePdfBytes);
        const copiedImgPages = await mergedPdf.copyPages(imgPdf, imgPdf.getPageIndices());
        copiedImgPages.forEach((page) => mergedPdf.addPage(page));
    }

    // Save final combined PDF
    const finalPdfBytes = await mergedPdf.save();
    download(finalPdfBytes, "combined_output_" + new Date().getTime() + ".pdf", "application/pdf");
}

document.querySelector('#combineButton').addEventListener('click', combineToPdf);
